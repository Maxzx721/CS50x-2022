...
Keep in mind, too, that you can read data from a file using [`fread`](https://manual.cs50.io/3/fread), which will read data from a file into a location in memory. Per its [manual page](https://manual.cs50.io/3/fread), `fread` returns the number of bytes that it has read, in which case it should either return `512` or `0`, given that `card.raw` contains some number of 512-byte blocks. In order to read every block from `card.raw`, after opening it with `fopen`, it should suffice to use a loop like:
...
-check50
- [x] :) recover.c exists.
- [x] :) recover.c compiles.
- [x] :) handles lack of forensic image
- [x] :) recovers 000.jpg correctly
- [x] :) recovers middle images correctly
- [x] :) recovers 049.jpg correctly
